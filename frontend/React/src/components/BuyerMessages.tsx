...
const areDatesSameDay = (date1: Date, date2: Date) => {
    return (
...
const BuyerMessages = () => {
    ...
    const getRoomId = queryParams.get('id')
    const toUserId = queryParams.get('tid')
    const getFromUserId = localStorage.getItem('fuser')
    useEffect(() => {
            ...
            if (token) {
                try {
                    ...
                    if (response) {
                        ...
                        localStorage.setItem('fuser', data.data[0].id)
    ...
    const getUserRooms = async (fromUserId: any) => {
        if (token) {
            try {

                const response = await fetch(`${configEnv.baseApiUrl}getUserRooms`, {
                    method: "post",
                    headers: {
                        "Content-Type": "application/json",
                    },
    ...
    const getData = async (fromUserId: any) => {
        if (token) {
            try {
                const response = await fetch(`${configEnv.baseApiUrl}getUserRooms`, {
                    method: "post",
                ...
                if (response) {
                    const data = await response.json();
                    let mactehdData = data.data.find(data => data.id == toUserId);
                    setTopProfileData(mactehdData);
                    setChatMessage('');
    ...
    useEffect(() => {
        getUserRooms(getFromUserId);
    }, [getFromUserId]);

    useEffect(() => {
        if (getRoomId) {
            joinWithReload(getRoomId);
        }
    }, [getRoomId]);

    const joinWithReload = (data: any) => {
        if (data) {
            const fromUserId = localStorage.getItem('fuser')
            getData(fromUserId);

            setSelectedChat(data);
            getOldMessagesFromDB(data);
            setRoom(data);
    ...
    // const getRoomaData = async (fromUserId: any) => {
    //     try {
    //         const response = await fetch(`${configEnv.baseApiUrl}getUserRooms`, {
    //             method: "post",
    //             headers: {
    ...
    //     }
    // };
    const getOldMessages = (data: any[]) => {
        const existingSentMessages = [...messageSent];
        const existingReceivedMessages = [...messageRecieved];
        let stringUserId = localStorage.getItem('fuser');
        let fromUserId = Number(stringUserId);
        setMessageSent([]);
        setMessageRecieved([]);
        data.forEach((message) => {
            const newMessage = {
                ...
                type: message.sender_id === fromUserId ? 'sent' : 'received',
            };

            if (message.sender_id === fromUserId) {
    ...
    const sendMessage = (e: { preventDefault: () => void; }) => {
        ...
        if (message !== '') {
            ...
            saveMessageInDatabase(message, room, fromUserId, time);
            setMessageSent([...messageSent, { message, timestamp: new Date(), type: 'sent' }]);
            socket.emit('sendMessage', { message, room, getToUserId: fromUserId, timestamp: new Date() });
            setMessage('');
        }
    };
    const saveMessageInDatabase = async (message: any, room: any, getToUserId: any, time: any) => {
        if (message) {
            try {
    ...
    useEffect(() => {
        if (formData) {
            const searchChat = async () => {
                setRoomData([]);
                try {
                    const response = await fetch(`${configEnv.baseApiUrl}searchChatByName`, {
                        method: "post",
                        headers: {
                            "Content-Type": "application/json",
                        },
                        body: JSON.stringify({ ...formData, fromUserId }),
                    });
    ...
    function searchUser(e: { target: { name: any; value: any; }; }, data: string) {
                ...
                <div className="hidden h-full flex-col xl:flex xl:w-1/4 bg-white">
                    ...
                    <div className="sticky border-b border-[#E0E0E0] px-[24px] py-[30px] pt-[32px]">
                        <h3 className="text-[#1D2D5C] font-[500] text-[18px] leading-[16px]">
                            Your Messages
                            {/* <span className="rounded-md border-[.5px] border-stroke bg-gray-2 py-0.5 px-2 text-base font-medium text-black dark:border-strokedark dark:bg-boxdark-2 dark:text-white 2xl:ml-4">7</span> */}
                        </h3>
                    </div>
                    <div className="flex max-h-full flex-col overflow-auto px-[24px] py-[30px]">
                        <form className="sticky mb-[30px]">
                            <input
                                ...
                                name='searchChat'
                                onChange={searchUser}
                                className="w-full rounded-[5px] bg-[#EFF4FB] py-[14px] pl-[19px] pr-10 text-[14px] outline-none focus:border-primary text-[#64748B] leading-[12px] placeholder:text-[#64748B] placeholder:text-[14px]"
                                placeholder="Search..."
                            />
                        ...
                        <div className="no-scrollbar max-h-full overflow-auto">
                                    ...
                                    >
                                        <div className="relative mr-[9px] h-[50px] w-full max-w-[50px] rounded-full border-[3px] border-[#fff] hover:border-[#fff]">
                                            {data.profile_picture ? (
                                                <img src={`${configEnv.baseApiUrl}${data.profile_picture}`} alt="profile" className="h-full w-full object-cover object-center rounded-full" />
                                            ) : (
                                        ...
                                        <div className="w-full">
                                            ...
                                            <div className="text-[#535D7A] text-[14px] font-[400] leading-[18px]">
                            ...
                            {getChatMessage && (
                                <span className="text-center">{getChatMessage && <p>{getChatMessage}</p>}</span>
                            )}
                        </div>
                        ...
                        <form className="flex items-center justify-between space-x-[20px]">
                            ...
                            <button onClick={sendMessage} className="flex h-[52px] w-full max-w-13 items-center justify-center rounded-[6px] bg-[#2174F5] text-white hover:bg-opacity-90">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M22 2L11 13" stroke="white" strokeWidth="2" stroke-linecap="round" stroke-linejoin="round"></path>
