...
interface FormData {
    ...
    profilePicture_old: FileList | null;
}


const AdminProfileInfo: React.FC = () => {
    const [errors, setErrors] = useState<Partial<FormData>>({});
    const [profileSuccessMessage, addSuccessMessage] = useState('');
    const [formData, setFormData] = useState<FormData>({
        name: '',
        email: '',
        description: '',
        profilePicture: null,
        profilePicture_old: null,
    });
    ...
    const fetchExistingData = async () => {
        try {
            ...
            if (response.ok) {
                ...
                const { name, email, profile_picture, description } = existingData.data;
                setFormData({
                    name: name,
                    email: email,
                    profilePicture: null,
                    profilePicture_old: profile_picture,
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        if (validateForm()) {
            try {
                ...
                // Get the profile picture files from the formData state
                const profilePictureFiles = formData['profilePicture'] ?? formData['profilePicture_old'];

                // Append profile picture files to formDataToSend
                if (profilePictureFiles) {

                    for (let i = 0; i < profilePictureFiles.length; i++) {
                        formDataToSend.append('admin_profile_picture', profilePictureFiles[i]);
                    }
                ...
                Object.entries(formData).forEach(([key, value]) => {
                    // Skip the profilePicture key, as we've already handled it above
                ...
                // Append token to formDataToSend
                formDataToSend.append('token', token);
                const response = await fetch(`${appUrl}updateAdminProfile`, {
                    method: 'POST',
                    body: formDataToSend,
                });

                if (response.ok) {
                    addSuccessMessage('Profile saved successfully!');
                ...
                <div className="rounded-sm bg-white dark:border-strokedark dark:bg-boxdark">
                    ...
                    <div className="flex flex-col gap-5.5 p-6.5">
                        ...
                        <div className=" gap-[28px]">
                            ...
                            <div className='pb-10'>
                                <label className="block text-black dark:text-white pt-10 pb-3">
                            ...
                            {profileSuccessMessage && (
                                <span className="bg-[green] text-center mt-[9px] max-w-[708px] w-full rounded-lg text-[green] py-[15px]">{profileSuccessMessage && <p>{profileSuccessMessage}</p>}</span>
                            )}
                        </div>
                    </div>
                </div>
